...
@register_model("bethe_gnn")
class BetheGNN(NodeModel):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
    ...
    def forward(
        ...
        if edge_index is None:
            raise ValueError("BetheGNN requires edge_index")
        src, dst = edge_index
        h = activation_layer(self.activation)(self.input_proj(x))
        edge_state = self.edge_init(torch.cat([h[src], h[dst]], dim=-1))
        for idx in range(self.num_layers):
            node_state = self.norms[idx](h)
            edge_delta = self.edge_update(
                torch.cat([node_state[src], node_state[dst], edge_state], dim=-1)
            )
            ...
            node_delta = self.node_update(
                torch.cat([node_state, incoming, outgoing, consistency], dim=-1)
            )
