...
class GATv2(NodeModel):
    def __init__(
        ...
        **_: object,
        ...
        self.convs.append(
            GATv2Conv(in_channels, hidden_channels, heads=heads, dropout=attention_dropout)
        )
        current = hidden_channels * heads
        self.norms.append(norm_layer(norm, current))
        for _ in range(num_layers - 1):
            self.convs.append(
                GATv2Conv(current, hidden_channels, heads=heads, dropout=attention_dropout)
            )
            current = hidden_channels * heads
            self.norms.append(norm_layer(norm, current))
    ...
    def forward(
        ...
        if edge_index is None:
            raise ValueError("GATv2 requires edge_index")
